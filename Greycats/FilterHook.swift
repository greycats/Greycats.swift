...
class Delegate: NSObject {
    ...
    var didCancel: (() -> Void)?
    var didSearch: ((UIResponder) -> Void)? = { $0.resignFirstResponder() }
...
open class FilterHook<T: Equatable> where T: Filtering {
    ...
    public init(data: Data, filter: Filter, shouldStart: (() -> Bool)? = nil, didCancel: @escaping () -> Void = {}, didSearch: ((UIResponder) -> Void)? = nil) {
    ...
    public convenience init(data: Data, filter: Filter, input: UITextField, shouldStart: (() -> Bool)? = nil, didSearch: ((UIResponder) -> Void)? = nil) {
        self.init(data: data, filter: filter, shouldStart: shouldStart, didSearch: didSearch)
        input.delegate = delegate
    }

    public convenience init(data: Data, filter: Filter, input: UISearchBar, shouldStart: (() -> Bool)? = nil, didCancel: @escaping () -> Void = {}, didSearch: ((UIResponder) -> Void)? = nil) {
